...
@Slf4j
@Component
public class NewsKafkaAdapter implements QuizRequestPort {

    private final KafkaTemplate<String, Object> kafkaTemplate;
    private final ObjectMapper objectMapper;
    private static final String TOPIC = "news-topic";

    public NewsKafkaAdapter(KafkaTemplate<String, Object> kafkaTemplate, ObjectMapper objectMapper) {
        ...
        try {
            ...
            kafkaTemplate.send(TOPIC, key, payload)
                .whenComplete((result, ex) -> {
                    if (ex == null) {
                        log.info("✅ [Kafka] news 전송 성공 | Key: {}, Partition: {}",
                                key, result.getRecordMetadata().partition());
                    } else {
                        log.error("❌ [Kafka] news 전송 실패 | Key: {} | Error: {}",
                                key, ex.getMessage());
                    }
                });
